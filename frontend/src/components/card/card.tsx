...
import "./style.css";
import { CardProps, TokenContent } from "@types";
...
const Card: React.FC<CardProps> = ({ data, deletePost, newError }) => {
  ...
  const tokenDecoded: TokenContent | null = useMemo(
    () => (token === null ? null : JSON.parse(atob(token.split(".")[1]))),
    [token],
  );
  ...
  return (
    <div className="card">
      ...
      <div className="cardUpDownButtons">
        ...
        {tokenDecoded !== null && data.posterId === tokenDecoded.ownerId ? (
          <>
            <BsDot />
